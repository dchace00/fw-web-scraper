...
def extract_thumbnail(soup):
    """Extract thumbnail image URL from the page."""
    thumbnail_url = None

    # Static fallback for first image in hero slideshow
    hero_image_tag = soup.select_one('.hero-image__slide img')
    if hero_image_tag and 'src' in hero_image_tag.attrs:
        thumbnail_url = hero_image_tag['src']
        if not thumbnail_url.startswith("http"):
            thumbnail_url = "https://www.f-w.com" + thumbnail_url
...
def scrape_project_data(url):
    ...
    project_data = {
        ...
        "Capabilities": [],
        "Markets": [],
        "Thumbnail": extract_thumbnail(soup)  # Incorporating thumbnail extraction
    }
